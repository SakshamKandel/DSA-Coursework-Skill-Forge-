...
<div class="p-2 sm:p-4 lg:p-10">
    <div class="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6 lg:gap-8 mb-6 lg:mb-10">
        <div class="flex items-center gap-4 sm:gap-6">
            <img src="<%= ctx %>/images/Adminn with student.png" alt="Mascot" class="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 object-contain shrink-0" />
            <div class="min-w-0">
                <h2 class="text-xl sm:text-2xl lg:text-3xl font-black text-slate-800 tracking-tight">Manage Students</h2>
                <p class="text-[0.7rem] font-black text-slate-400 uppercase tracking-widest mt-2">View, unlock and remove registered students</p>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse min-w-[760px]">
                <thead>
                    <tr class="bg-slate-50/50">
                        <th class="px-5 sm:px-8 py-4 lg:py-6 text-[0.7rem] font-black text-slate-400 uppercase tracking-widest">Name</th>
                        <th class="px-5 sm:px-8 py-4 lg:py-6 text-[0.7rem] font-black text-slate-400 uppercase tracking-widest">Contact</th>
                            ...
                            <div class="flex items-center gap-3 sm:gap-5">
                                <div class="relative shrink-0">
                                    <% if (st.getProfilePhoto() != null && st.getProfilePhoto().length > 0) { %>
                                        <img src="<%= ctx %>/photo?userId=<%= st.getId() %>" alt="Photo" class="w-12 h-12 sm:w-16 sm:h-16 rounded-2xl sm:rounded-[1.2rem] object-cover bg-white ring-4 ring-slate-50 border border-slate-100 shadow-sm" />
                                    <% } else { %>
                                        <div class="w-12 h-12 sm:w-16 sm:h-16 rounded-2xl sm:rounded-[1.2rem] bg-brand/10 flex items-center justify-center text-lg sm:text-xl font-black text-brand ring-4 ring-brand/5 border border-brand/10 shadow-sm">
                                ...
                                <div class="min-w-0">
                                    <div class="text-sm sm:text-base font-black text-slate-800 leading-tight truncate"><%= st.getFullName() %></div>
                        ...
                        <td class="px-5 sm:px-8 py-5 lg:py-8">
                            <div class="flex flex-col gap-1.5">
                                <div class="flex items-center gap-2 text-xs sm:text-sm font-bold text-slate-600">
                                    <svg class="w-3.5 h-3.5 text-slate-300 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><rect width="20" height="16" x="2" y="4" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>
                                    <span class="truncate"><%= st.getEmail() %></span>
                                </div>
                                <div class="flex items-center gap-2 text-[0.7rem] font-bold text-slate-400">
                                    <svg class="w-3.5 h-3.5 text-slate-200 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                                    <%= st.getPhone() %>
                                </div>
                            </div>
                        </td>
                            ...
                            <div class="flex items-center justify-end gap-2 sm:gap-3">
                                <% if (st.isLocked()) { %>
                                    <a href="<%= ctx %>/admin?action=unlock&id=<%= st.getId() %>"
                                       class="px-3 sm:px-5 py-2 sm:py-3 text-[0.65rem] font-black uppercase tracking-widest text-amber-600 bg-amber-50 rounded-xl hover:bg-amber-100 transition-all border border-amber-100 whitespace-nowrap">
                                        Unlock
                                    </a>
                                <% } %>
                                <a href="<%= ctx %>/admin?action=remove&id=<%= st.getId() %>"
                                   class="p-2 sm:p-3 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all"
                ...
                <% } } else { %>
                    <tr><td colspan="4" class="px-5 sm:px-8 py-20 lg:py-32 text-center">
                        <div class="w-20 h-20 lg:w-24 lg:h-24 bg-slate-50 rounded-3xl lg:rounded-[2.5rem] flex items-center justify-center mx-auto mb-6 lg:mb-8 text-slate-100">
                            <svg class="w-10 h-10 lg:w-12 lg:h-12" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="m16 3.13a4 4 0 0 1 0 7.75"/></svg>
                        </div>
                        <p class="text-base font-black text-slate-400 uppercase tracking-widest">No students found</p>
                    </td></tr>
                <% } %>
                </tbody>
            </table>
