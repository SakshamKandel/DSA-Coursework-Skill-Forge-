...
<div class="p-2 sm:p-4 lg:p-10 space-y-8 lg:space-y-12">
    ...
    <div class="mascot-bubble-container">
        ...
        <div class="speech-bubble">
            <p>Welcome back, <%= userName %>. Let us continue your learning today.</p>
        </div>
    </div>

    <!-- Summary statistic cards -->
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-10">
        <!-- Enrolled count -->
        <div class="bg-white p-6 sm:p-8 lg:p-10 rounded-3xl lg:rounded-[3rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] ring-1 ring-slate-100 flex flex-col justify-between">
            <div class="flex items-center gap-4 mb-6 lg:mb-8">
                <div class="w-10 h-10 sm:w-12 sm:h-12 shrink-0 rounded-2xl bg-blue-50 text-blue-500 flex items-center justify-center">
                    <svg class="w-5 h-5 sm:w-6 sm:h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20"/><path d="M8 7h6"/><path d="M8 11h4"/></svg>
                </div>
                <h3 class="text-[0.7rem] font-black text-slate-400 uppercase tracking-widest">Enrolled</h3>
            </div>
            <div class="text-4xl sm:text-5xl lg:text-6xl font-black text-slate-800 tabular-nums"><%= enrolledCount %></div>
        </div>

        <!-- Completed count -->
        <div class="bg-white p-6 sm:p-8 lg:p-10 rounded-3xl lg:rounded-[3rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] ring-1 ring-slate-100 flex flex-col justify-between">
            <div class="flex items-center gap-4 mb-6 lg:mb-8">
                <div class="w-10 h-10 sm:w-12 sm:h-12 shrink-0 rounded-2xl bg-amber-50 text-amber-500 flex items-center justify-center">
                    <svg class="w-5 h-5 sm:w-6 sm:h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg>
                </div>
                <h3 class="text-[0.7rem] font-black text-slate-400 uppercase tracking-widest">Completed</h3>
            </div>
            <div class="text-4xl sm:text-5xl lg:text-6xl font-black text-slate-800 tabular-nums"><%= completedCount %></div>
        </div>

        <!-- Browse courses call to action -->
        <div class="bg-slate-900 p-6 sm:p-8 lg:p-10 rounded-3xl lg:rounded-[3rem] shadow-xl shadow-slate-900/10 flex flex-col justify-between group overflow-hidden relative sm:col-span-2 lg:col-span-1">
            <div class="absolute inset-0 bg-[radial-gradient(circle_at_30%_-20%,#58cc0220_0%,transparent_50%)]"></div>
            <div class="relative z-10">
                <h3 class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest mb-2">Available Courses</h3>
                <p class="text-white font-bold text-base sm:text-lg leading-tight mb-6 sm:mb-8">Browse the catalog and enroll in a new course.</p>
            </div>
            <a href="<%= ctx %>/student/courses" class="relative z-10 w-full bg-brand text-white py-4 sm:py-5 rounded-2xl font-black text-sm uppercase tracking-widest flex items-center justify-center gap-3 shadow-[0_6px_0_0_rgb(66,153,2)] hover:translate-y-[-2px] active:translate-y-[2px] active:shadow-none transition-all">
                Browse Courses
    ...
    <section>
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6 lg:mb-8">
            <div>
                <h2 class="text-xl sm:text-2xl font-black text-slate-800 tracking-tight">My Courses</h2>
                <p class="text-[0.7rem] font-black text-slate-400 uppercase tracking-widest mt-2">Your enrolled courses and progress</p>
            </div>
        </div>
                ...
                <table class="w-full text-left border-collapse min-w-[680px]">
                    <thead>
                        <tr class="bg-slate-50/50">
                            ...
                            <th class="px-5 sm:px-8 lg:px-10 py-4 lg:py-6 text-[0.7rem] font-black text-slate-400 uppercase tracking-widest text-right">Enrolled On</th>
                        </tr>
                    </thead>
                    ...
                    <% if (enrollments != null && !enrollments.isEmpty()) {
                        ...
                        <tr class="group hover:bg-slate-50/30 transition-colors text-sm">
                            ...
                            <td class="px-5 sm:px-8 lg:px-10 py-5 lg:py-8 w-48 lg:w-64">
                                <div class="flex items-center gap-3 lg:gap-4">
                                    <div class="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden p-[2px]">
                                        <div class="js-progress-fill h-full rounded-full transition-all duration-1000 ease-out <%= en.getProgress() >= 100 ? "bg-amber-400" : "bg-brand" %> shadow-sm" data-progress="<%= en.getProgress() %>"></div>
                                    </div>
                    ...
                    <% } } else { %>
                        <tr><td colspan="4" class="px-5 sm:px-8 lg:px-10 py-16 lg:py-24 text-center">
                            ...
                            <p class="text-sm font-black text-slate-400 uppercase tracking-[0.2em]">No courses enrolled yet</p>
                        </td></tr>
                    <% } %>
                    </tbody>
                </table>
