...
/**
 * Serves the profile pages for both admin and student users.
 * Supports updating the basic profile fields, changing the password and
 * uploading or removing the profile photo.
 */
@WebServlet(urlPatterns = {"/student/profile", "/admin/profile"})
@MultipartConfig(
        fileSizeThreshold = 1024 * 1024 * 2,    // 2 MB before spilling to disk
        maxFileSize       = 1024 * 1024 * 50,   // 50 MB per file
        maxRequestSize    = 1024 * 1024 * 60    // 60 MB per request
)
public class ProfileServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        // Reload the user and forward back to the profile page either way
        try {
            User user = userService.findById(userId);
            req.setAttribute("userBean", user);
    ...
    /**
     * Reads the original filename out of the content-disposition header.
     */
